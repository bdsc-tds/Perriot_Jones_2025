...
# Merge and plot umap prior to batch correction ----

umap_ndim <- 20
umap_unintegrated <- sprintf("umap_unintegrated_%s_dim.pdf", umap_ndim)

if (! file.exists(umap_unintegrated)){
    ...
    seurat_obj <- RunUMAP(seurat_obj, dims = 1:umap_ndim)

    pdf(umap_unintegrated)
...
sct_objs <- lapply(seurat_objs, function(obj){
    SCTransform(obj,
...
names(sct_objs) <- names(exp_dirs)

features <- SelectIntegrationFeatures(object.list = sct_objs,
                                      nfeatures = n_integration_features)

sct_objs <- PrepSCTIntegration(object.list = sct_objs,
                               anchor.features = features)

anchors <- FindIntegrationAnchors(object.list = sct_objs,
                                  normalization.method = "SCT",
                                  anchor.features = features)

# Anchors file exists in data/processed
write_rds(anchors, file = "anchors.rds")

#all_features <- unique(unlist(lapply(sct_objects, Features)))

integrated <- IntegrateData(anchorset = anchors,
                            normalization.method = "SCT")#,
                            #features.to.integrate = all_features)

write_rds(integrated, file = "integrated.rds")
